...
ヘッダ右側にはアプリ切替タブ + 状態表示 (`K +n` 転調値 / `PLAY MM:SS` 経過時間) と、`MIX` 入力ソース選択ボタン、`BASE` (転調基準ピッカー)、`CONF` (`/config.json` エディタ) の専用ボタンが並ぶ。ツールバー (2 段目) は各アプリのサブモードタブ + トランスポート/補助ボタン専用で、横幅を有効に使えるレイアウトにしてある。
...
- ヘッダ右の **CONF** ボタン (新) → `CONFIG_EDIT_MODE` を直接開く。フィールドをタップで巡回、4 件 × 3 ページ + footer の SAVE / CANCEL / APPLY。`PAGE-` / `PAGE+` で 3 ページ間を往復。
- ヘッダ右の **BASE** ボタン (新) → `BASE_SET_MODE` (3 ページの転調基準ピッカー: -12..-1 / -5..+6 / +1..+12)。数字は `XPOSE/DIRECT` と同じ大きさ (FONT_HUGE) で **黄色**で表示し、選択中の値は緑バックの黒文字。`+6` → R / `-5` → L / `+1` → M / `-1` → M / `PAGE` で巡回 / `EXIT` で戻る。
